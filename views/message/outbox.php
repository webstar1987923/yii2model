...
<div class="wrap cf mt20">
	<?= LeftMenuMessageWidget::widget(['active' => 'outbox']) ?>
    		...
    		<tbody>
    			<?php if (count($userMessages)) { ?>
	    			<?php foreach ($userMessages as $userMessage) { ?>
			    		<tr>
			    			<td><?php 
				    				echo $userMessage->userReceive->user_name; 
				    				if (count($userMessage->messages)) {
				    					echo "(".count($userMessage->messages).")";
				    				}
				    			?>
			    			</td>
			    			<td><?= $userMessage->lastMessageOut->subject ?></td>
			    			<td><?= $userMessage->update_at ?></td>
			    			<td class='action'>
			    				<a href="/message/view?message_id=<?= $userMessage->message_id ?>">View</a>
			    				<a class='delete_button' href="/message/delete?message_id=<?= $userMessage->message_id ?>&placeholder=2">Delete</a>
			    			</td>
			    		</tr>
		    		<?php } ?>
					<script>
